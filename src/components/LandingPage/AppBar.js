...
function NavLinkButton({  path,   //path that the button corresponds to. Always keep this in camelcase
                          label,  //label of the button
                          buttonTextColor, buttonTextColorOnActive, 
                          buttonBackgroundColor, buttonBackgroundColorOnActive, 
                          buttonBorderRadius, 
                          activeButtonColorOnHover, inactiveButtonBackgroundColorOnHover, parentPath}) {   //the other names are self-explanatory
  ...
  const style = {
    ...
    backgroundColor: isActive ? buttonBackgroundColorOnActive : buttonBackgroundColor, // Active or transparent background
    borderRadius: buttonBorderRadius,
    '&:hover': {
      backgroundColor: isActive ? activeButtonColorOnHover : inactiveButtonBackgroundColorOnHover, // Darker blue on hover if active, otherwise light blue
    },
    fontFamily: TITLE,
    fontWeight: 'bold',
    padding: 1.5,
...
function AppBar({ paths,    //array of paths that correspond to the buttons in the appbar. Alwawys keep these in camelcase
    ...
    <MuiAppBar position="fixed" sx={{
      ...
      <Toolbar sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
        <img 
          src={appBarLogoSrc}
          alt="Logo" 
          style={{ marginRight: 8, width: 340, marginTop: 5, marginBottom: 5 }}
        />
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {paths.map((path, index) => (
            <NavLinkButton
              key={index}
              path={path}
              label={path === '/' ? defaultPathName : camelCaseToTitle(path.slice(1))}
              ...
              inactiveButtonBackgroundColorOnHover={inactiveButtonBackgroundColorOnHover}
